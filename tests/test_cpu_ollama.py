"""CPU-only Ollama construction contracts for the local capstone demo."""

from __future__ import annotations


def test_all_ollama_clients_explicitly_disable_gpu(monkeypatch) -> None:
...
def test_cpu_only_is_the_default_runtime_policy() -> None:
    from stai.config import Settings

    local_settings = Settings(_env_file=None)
    assert local_settings.ollama_num_gpu == 0
    assert local_settings.agent_model == "llama3.2:latest"
