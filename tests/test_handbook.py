...
from stai.handbook import build_handbook, load_source, verify_publication


def test_page_native_source_and_108_page_publication(tmp_path: Path) -> None:
    source = load_source()
    assert source.handbook_version == "1.0"
...
def test_handbook_build_is_deterministic_and_rag_pages_match_manifest(tmp_path: Path) -> None:
    ...
    assert all(row["handbook_version"] == "1.0" for row in rag_pages)
