...
`PolicyTurnEngine.handle_turn(conversation_id, message)` is the one production seam used by Streamlit, FastAPI, and turn-level tests. It loads a bounded safe context, resolves references and dialogue acts, then selects a deterministic workflow action or a policy path. The ReAct layer is rebuilt for policy turns and can use only five bounded operations: resolve the active handbook, search eligible handbook records, evaluate applicability, look up bounded public holidays, and offer an escalation route. Tool arguments are schema validated. `RunCapture` records closed tool identities and evidence metadata so output validation and observability do not depend on parsing model prose. State-changing operations such as case consent, attribute approval, sharing, revocation, and deletion are deterministic application commands rather than agent tools.

The active Chroma adapter supplies dense candidates to weighted lexical retrieval. Stopwords do not determine rank, the resolved onboarding topic is a hard candidate gate, and evidence is grouped within eligible policy pages. If Ollama is unavailable, a deterministic composer may use only the same verified, topic-gated evidence. A candidate with a valid citation shape but a citation from another topic is rejected before display.
...
SQLite is used as deterministic application state, not as an LLM-generated SQL surface. Schema epoch 3 adds safe typed turn results—dialogue act, resolved topic, execution mode, reference identity, and the public response payload—to the existing normalized profile, conversation, citation, escalation, validation, retrieval, retry, and idempotency state. It stores no retrieved snippets, rewritten queries, or hidden reasoning. Connection-per-operation behavior remains suitable for Streamlit threads.
...
Consent is represented as deterministic state progression. Requesting help creates or returns an escalation offer and shows its bounded summary. Explicit language such as “route it please” counts only when that pending summary has already been shown; the application—not the model—then executes the same versioned consent command used by the dedicated endpoint. HR closes a case with the expected resource version, preventing a stale update from overwriting a newer state. Attribute changes follow the same optimistic version pattern and permit only one attribute per request.
