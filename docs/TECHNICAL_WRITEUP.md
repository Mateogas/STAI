...
SQLite is used as deterministic application state, not as an LLM-generated SQL surface. Schema epoch 6 adds Case Information Requests and separately consented direct-conversation mode to Evidence Gaps, typed Case Resolution Memory, Case Threads, messages, events, and notifications. It stores no retrieved snippets, rewritten queries, or hidden reasoning. Connection-per-operation behavior remains suitable for Streamlit threads.
