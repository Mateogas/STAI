...
The refactor described here replaces a broad assistant concept with a narrow set of enforceable contracts. A policy response must be one of four typed outcomes. Applicability is determined from four HR-confirmed profile attributes. Evidence comes from one active immutable 108-page handbook build. Human routing requires explicit consent. Certificate checking is a local completeness-only process with result-only history. Streamlit and FastAPI use one shared service layer. Privacy-safe schema-v2 operation metadata travels through the existing protected observability topology. A frozen 60-case safety benchmark and a Linux container smoke make those claims reproducible.
...
Privacy requirements are equally central. Policy conversation text is private to the Hire surface and cannot be read from HR endpoints. Informal chat cannot modify the authoritative profile. An escalation offer does not itself create a case. HR sees a case only after explicit consent, and receives a bounded summary rather than the transcript. Certificate content is more restricted: it must be routed away from policy chat before persistence, processed locally, and omitted from public history and telemetry. Monitoring must be operationally useful without introducing stable employee tracking or content collection.
...
Policy interaction returns a discriminated `PolicyResponse`. `GroundedAnswer` contains validated claims and structured citations. `ClarificationRequest` asks one focused question when an unknown constraining profile attribute can change applicability. `Abstention` states that eligible evidence cannot support an answer. `EscalationOffer` presents an appropriate human route and a privacy-safe proposed summary. These shapes prevent downstream code from guessing the meaning of arbitrary text.
...
The ReAct layer is rebuilt for each turn and can use only five bounded operations: resolve the active handbook, search eligible handbook records, evaluate applicability, look up bounded public holidays, and offer an escalation route. Tool arguments are schema validated. `RunCapture` records closed tool identities and evidence metadata so output validation and observability do not depend on parsing model prose. State-changing operations such as case consent, attribute approval, sharing, revocation, and deletion are deterministic service or repository commands rather than agent tools.

Input classification is allowed to fail open because an unavailable small classifier should not make the entire product unavailable. This does not weaken consequential gates. Output schema validation, evidence eligibility, applicability, citation validation, consent, versions, and certificate privacy all fail closed. Malformed model output becomes a typed abstention. Off-topic and prompt-injection classifications return scoped results without exposing system instructions.
...
SQLite is used as deterministic application state, not as an LLM-generated SQL surface. Normalized tables represent the active handbook pointer, immutable builds and records, profile revisions, policy conversations and messages, escalation offers and cases, attribute change requests, validation results, retry state, Nager cache, idempotency records, and telemetry shipping state. Connection-per-operation behavior remains suitable for Streamlit threads.
...
Consent is represented as state, not inferred intent. Requesting a person creates or returns an escalation offer. The consent endpoint creates the case and records only the bounded proposed summary. HR closes a case with the expected resource version, preventing a stale update from overwriting a newer state. Attribute changes follow the same optimistic version pattern and permit only one attribute per request.
...
Streamlit presents role and destination separately. The Hire can choose Ask AISHA, Certificate Check, or History; HR User sees structured Cases, Attribute Change Requests, and shared Validation Results. The UI avoids a transcript-centric HR dashboard. Visible status text does not rely on color alone, controls are designed for keyboard access and 44-pixel targets, dynamic outcomes use status regions, and the layout is exercised at 320 CSS pixels as well as desktop width.

FastAPI exposes only `/api/v1`. The contract includes health; conversation create, list, message, detail, and delete; escalation consent; Hire and HR case operations; attribute request and HR decision operations; certificate validate and retry; validation-result list, detail, share, revoke, delete; and HR shared-result reads. Legacy unversioned paths are absent. OpenAPI uses typed request and response models, consistent `{data, meta}` and `{error, meta}` envelopes, privacy-safe errors, configured CORS, Alyssa-only demo namespaces, resource versions, cursor bounds, and idempotency semantics.

Both surfaces call the same `AishaService` and repository. This is important for acceptance: the UI cannot quietly provide a weaker consent rule or richer HR data than the API. Tests compare surface outcomes, inspect OpenAPI for forbidden fields, replay idempotent messages, close cases, revise attributes, and exercise the complete certificate lifecycle.
