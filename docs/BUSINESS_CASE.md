...
| Human support | Eligible partial evidence and a material gap are required before a sharing notice and consented AISHA-mediated child Case Thread |
...
| HR visibility | Consented Mediated Cases, attribute requests, and currently shared safe validation-result metadata—never unrelated policy chat or certificate contents |

## Privacy and trust design

Trust is part of the product, not a disclaimer added at the end. HR cannot browse Alyssa’s Policy Conversations, and a bare request for a human cannot create HR work. AISHA first proves that eligible evidence answers part of the question and identifies a material gap. When Alyssa creates the resulting case, the consent screen explicitly states that the linked parent history and its future messages will be copied into an HR-visible child Case Thread until resolution. HR asks missing questions through AISHA and decides a typed resolution; it enters direct conversation only after a separate consent. Other conversations remain private. An HR resolution is immediately reusable only within that thread; broader clarification reuse requires review, while exceptions remain case-scoped and rule changes require a handbook revision. Chat cannot silently convert an informal statement into an authoritative profile fact.
