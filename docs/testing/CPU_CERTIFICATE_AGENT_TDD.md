...
Integration note: this document records the original `testing` branch run.
When merged into current `main`, the shared Ollama construction seam retained
that explicit CPU-only option while restoring automatic hardware acceleration
as the default and preserving `llama3.1:8b` as the production agent model.
