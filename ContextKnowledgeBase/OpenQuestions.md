...
The issue #1 child-ticket decisions are closed and authoritative. Do not reopen them during implementation or presentation.
...
- Public integration is only typed `/api/v1`; `/chat` and unversioned `/health` have no compatibility aliases.
- Telemetry is closed operational metadata over the protected four-stage topology and cannot reconstruct Alyssa activity.
- SQL Agent is unclaimed; Chroma RAG is the selected retrieval module.
- The frozen Composite Safety Benchmark and final module matrix determine acceptance; older ad hoc guardrail scores do not.

Future enhancements require a new explicit decision and must preserve every privacy, applicability, evidence, consent, reset, and telemetry hard gate.
