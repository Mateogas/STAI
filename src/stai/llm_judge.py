    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        if not agent_enabled:
            try:
                ...
                raise RuntimeError(
                    "--offline-agent requires an AISHA source checkout with tests"
                ) from exc
            ...
            service_kwargs.update(
                ...
                agent_runner=OfflineReactRunner(repo, records, index),
                input_classifier=OfflineClassifier(),
            )
        service = AishaService(repo, records, **service_kwargs)
